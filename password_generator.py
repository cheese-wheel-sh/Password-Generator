...
def generate(mode, chars, length, safemode):
    timer_st('generate')
    if mode == 'weak':
        print('No safemode for weak available!')
        time.sleep(1)
    if safemode in ('Yes', 'yes', 'Y', 'y'):
        ...
        if mode == 'strong':
            ...
            intgen = re.findall(r'\d+', str(password))
            while len(lettergenerator) < (length / 3) and len(lettergenerator) > (length / 2):
                clear()
                tries()
                password = ''.join(random.choice(chars) for x in range(length))
                lettergenerator = re.findall("[A-Z]+", str(password))
            while len(intgen) < (length / 3) and len(intgen) > (length / 2):
                clear()
                tries()
                password = ''.join(random.choice(chars) for x in range(length))
                intgen = re.findall(r'\d+', str(password))
